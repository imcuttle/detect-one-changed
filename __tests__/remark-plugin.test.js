...
const md = require('../remark-plugin')
const { fixture, read } = require('./helper')

const fakePath = fixture('fake.md')

describe('mdx - use remark-plugin', function() {
...
`)
    expect(md.cache.has(fakePath)).toBeTruthy()
  })
...
`)
  ...
  function mdxDiff(name) {
    const mdPlugin = md.create()
    mdx.sync(read(`remark-plugin/${name}/old.md`), {
      filepath: fakePath,
      mdPlugins: [mdPlugin]
    })

    return mdx.sync(read(`remark-plugin/${name}/new.md`), {
      filepath: fakePath,
      mdPlugins: [mdPlugin]
    })
  }
...
`)
  ...
  it('should mdx diff `readme`', function() {
    expect(mdxDiff('readme')).toMatchSnapshot()
  })
})
